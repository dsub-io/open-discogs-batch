...
/**
 * Simple class to perform testings to ensure compatibility. For internal use only.
 */
@Slf4j
public class DefaultDatabaseConnectionValidator implements DatabaseConnectionValidator {
   ...
   * validation will be depending on following option argument values: 1. url 2. username 3.
   * password
   *
   * <p>the argument names must match, and should be present in <i><b>optional</b></i> argument. if
   * multiple values are present, only the first value will be examined.
   *
   * @param args a {@link ApplicationArguments} that contains url, username, password as option
   *             argument.
   * @return validation result.
   */
  @Override
   ...
   *
   * <p>failure points can be one of the followings:
   *
   * <ul>
   *   <li>if database product is unknown
   *   <li>if driver acquisition failed
   *   <li>if connection acquisition failed
   *   <li>if database metadata acquisition failed
   *   <li>if the metadata shows incompatibility
   * </ul>
   *
